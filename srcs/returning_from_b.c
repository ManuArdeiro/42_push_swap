...
/*   Updated: 2022/10/19 20:18:51 by jolopez-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int argc)
{
	...
	while (ft_hits_count(stack_a) < (argc - 2))
	{
		...
		if (stack_b[0].pos < stack_a[0].pos && stack_b[0].pos > stack_a
			[ft_stack_last_element(stack_a)].pos)
			ft_push_a(stack_a, stack_b, moves);
		else if (stack_b[0].pos < stack_a[0].pos && stack_a
			[ft_stack_last_element(stack_a)].value == max[0])
			ft_push_a(stack_a, stack_b, moves);
		else
			ft_reverse_rotate_a(stack_a, moves, 'Y');
	}
